...
class TweetManager:
    ...
    async def get_tweetdata(self, message_id: int) -> Union[None, TweetParameter]:
        ...
        async with AsyncSession(engine) as session:
            async with session.begin():
                stmt = select(TweetObj).where(TweetObj.message_id == message_id)
    ...
    async def remove_tweetdata(self, message_id: int) -> None:
        ...
        async with AsyncSession(engine) as session:
            async with session.begin():
                stmt = delete(TweetObj).where(TweetObj.message_id == message_id)
...
if __name__ == "__main__":
    ...
    test = TweetParameter(
        message_id=12334, channel_id=123, author_id=123, content="hoge"
    )
