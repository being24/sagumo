...
class MyModal(commands.Cog, name="Modal管理用cog"):
    ...
    @app_commands.command(
        name="proxy_transmission", description="代理投稿を行うコマンド"
    )
            ...
            async def on_submit(self, interaction: discord.Interaction):
                if not isinstance(interaction.channel, discord.abc.Messageable):
                    await interaction.response.send_message(
                        "テキストチャンネル限定です", ephemeral=True
                    )
                    return
                await interaction.response.send_message(
                    "代理投稿を行いました", ephemeral=True
                )
                try:
                    await interaction.channel.send(self.answer.value)
                except discord.Forbidden:
                    await interaction.response.send_message(
                        "メッセージの送信に失敗しました。Forbidden", ephemeral=True
                    )
                except discord.HTTPException:
                    await interaction.response.send_message(
                        "メッセージの送信に失敗しました。HTTPException", ephemeral=True
                    )
    ...
    async def edit_message(
        ...
        if self.bot.user.id != message.author.id:
            await interaction.response.send_message(
                "自分のメッセージのみ編集できます", ephemeral=True
            )
            return
    ...
    async def disposition_record(
        ...
        if user is None:
            await interaction.response.send_message(
                "ユーザーが見つかりませんでした", ephemeral=True
            )
            return

        class ProxyModal(Modal, title="処分記録を投稿します"):
            desc = TextInput(label="概要", style=discord.TextStyle.paragraph)
            corresponding_part = TextInput(
                label="該当箇所", style=discord.TextStyle.paragraph
            )
            discussion_part = TextInput(
                label="議論箇所", style=discord.TextStyle.paragraph
            )
            ...
            async def on_submit(self, interaction: discord.Interaction):
                ...
                embed = discord.Embed(
                    title=f"{today}",
                    description=f"desc : {self.desc.value}",
                    color=discord.Color.red(),
                )
                embed.add_field(
                    name="・該当箇所", value=self.corresponding_part.value, inline=False
                )
                embed.add_field(
                    name="・議論箇所", value=self.discussion_part.value, inline=False
                )
                embed.add_field(name="・対応", value=self.deal.value, inline=False)
                embed.set_author(
                    name=f"{user.name} ID:{user.id}", icon_url=user.display_avatar
                )

                await interaction.channel.send(embed=embed)

                await interaction.response.send_message(
                    "処分記録を行いました", delete_after=5, ephemeral=True
                )
    ...
    async def disposition_record_error(self, interaction: discord.Interaction, error):
        if not isinstance(interaction.channel, discord.abc.Messageable):
            self.logger.info(
                f"{interaction.channel}はメッセージ送信できないチャンネルです{error}"
            )
            return
